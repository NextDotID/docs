...
+ Request (application/json)
  ...
  + Attributes (object)
    ...
    + platform (string, required) - Target platform. See table above for all available platforms. See table in [Platform supported](https://github.com/NextDotID/proof_server#supported) for all available values.
    + identity (string, required) - Identity in target platform to proof. Usually a "username" or "screen name". See [Platform supported](https://github.com/NextDotID/proof_server#supported).
...
+ Request (application/json)
  ...
  + Attributes (object)
    ...
    + identity (string, required) - Identity in target platform to proof. Usually a "username" or "screen name". See [Platform supported](https://github.com/NextDotID/proof_server#supported).
    + proof_location (string, optional) - Location where public-accessible proof post is set. See [Platform supported](https://github.com/NextDotID/proof_server#supported).
    ...
    + extra (object, optional) - Extra info for specific platform needed. See [Flow](/introduction/how-it-works#binding-an-avatar-to-an-application-via-ethereum-wallet) for more info.
...
Found.
  ...
  + Attributes
    ...
    + proof_chain (array[object], required) - Will be empty array if not found.
        ...
        + platform (string, required) - Target platform. See table above for all available platforms. See table above for all available values.
        + identity (string, required) - Identity on that platform
        + proof_location (string, required) - Location where public-accessible proof post is set. See [Platform supported](https://github.com/NextDotID/proof_server#supported).
        + created_at (string, required) - Creation time of this proof. (timestamp, unit: second)
...
Found.
  ...
  + Attributes
    ...
    + links (array[object], required) - Each link of every ProofChain.
        ...
        + action (string, required) - Action (`create` / `delete`)
        + platform (string, required) - Target platform. See [Platform supported](https://github.com/NextDotID/proof_server#supported) for all supported platforms.
        + identity (string, required) - Identity on that platform. See [Platform supported](https://github.com/NextDotID/proof_server#supported) for the meaning of this field..
        + proof_location (string, required) - Location where public-accessible proof post is set. See [Platform supported](https://github.com/NextDotID/proof_server#supported).
        + created_at (string, required) - Creation time of this proof. (timestamp, unit: second)
        + signature (string, required) - generate signature_payload and avatar_private_key
        + signature_payload (string, required) - Raw string to be sent to `personal_sign`
